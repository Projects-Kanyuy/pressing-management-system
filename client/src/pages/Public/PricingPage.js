...
const CheckListItem = ({ children }) => (
    <li className="flex items-start">
        ...
        <span className="text-apple-gray-600 dark:text-apple-gray-300">{children}</span>
    </li>
);
...
const PricingPage = () => {
    ...
    const findPriceString = (plan) => {
        ...
        if (['XAF', 'XOF'].includes(priceToShow.currency)) {
            return `${priceToShow.amount.toLocaleString('fr-FR')} ${priceToShow.currency}`;
        }
        // A more generic currency formatting
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-5xl mx-auto items-stretch">
                           ...
                           {basicPlan && (
                                <div className="border dark:border-apple-gray-700 rounded-apple-xl p-8 flex flex-col bg-white dark:bg-apple-gray-800/50">
                                    <h3 className="text-2xl font-semibold">{t('public.pricing.plans.basic.name')}</h3>
                                    <p className="mt-4 text-5xl font-bold tracking-tight">
                                        {findPriceString(basicPlan)}
                                        <span className="ml-1 text-xl font-medium text-apple-gray-500 dark:text-apple-gray-400">/{t('public.pricing.plans.basic.frequency')}</span>
                                    </p>
                                    ...
                                    <Link to={`/signup?plan=basic`} className="w-full mt-auto">
                                        <Button variant="secondary" size="lg" className="w-full">{t('public.pricing.choose')} {t('public.pricing.plans.basic.name')}</Button>
                                    </Link>
                                </div>
                           )}
                           ...
                           {proPlan && (
                               <div className="relative p-8 rounded-apple-xl border-2 border-apple-blue shadow-apple-xl flex flex-col bg-white dark:bg-apple-gray-800/50">
                                   <div className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2">
                                       <span className="inline-flex items-center px-4 py-1 rounded-full text-sm font-semibold text-white bg-apple-blue">{t('public.pricing.mostPopular')}</span>
                                   </div>
                                   <h3 className="text-2xl font-semibold">{t('public.pricing.plans.pro.name')}</h3>
                                   <p className="mt-4 text-5xl font-bold tracking-tight">
                                       {findPriceString(proPlan)}
                                       <span className="ml-1 text-xl font-medium text-apple-gray-500 dark:text-apple-gray-400">/{t('public.pricing.plans.pro.frequency')}</span>
                                   </p>
                                   <ul className="mt-6 space-y-3 mb-8">
                                       {t('public.pricing.plans.pro.features', { returnObjects: true }).map((feature, i) => <CheckListItem key={i}>{feature}</CheckListItem>)}
                                   </ul>
                                   <div className="flex-grow"></div>
                                   <Link to={`/signup?plan=pro`} className="w-full mt-auto">
                                       <Button variant="primary" size="lg" className="w-full">{t('public.pricing.choose')} {t('public.pricing.plans.pro.name')}</Button>
                                   </Link>
                               </div>
                           )}
                           ...
                           {enterprisePlan && (
                                <div className="border dark:border-apple-gray-700 rounded-apple-xl p-8 flex flex-col bg-white dark:bg-apple-gray-800/50">
                                    <h3 className="text-2xl font-semibold">{t('public.pricing.plans.enterprise.name')}</h3>
                                    <p className="mt-4 text-4xl font-bold">
                                        {t('public.pricing.plans.enterprise.price')}
                                        <span className="ml-1 text-lg font-medium text-apple-gray-500 dark:text-apple-gray-400">/{t('public.pricing.plans.enterprise.frequency')}</span>
                                    </p>
                                    <ul className="mt-6 space-y-3 mb-8">
                                    ...
                                    <Link to="/contact" className="w-full mt-auto">
                                        <Button variant="secondary" size="lg" className="w-full">{t('public.pricing.contactSales')}</Button>
                                    </Link>
                                </div>
                           )}
                        </div>
