...
const DirectoryAdminLoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            await dirAdminLogin({ username: username, 
                email: username, // Sending both ensures it works regardless of which one the backend checks
                password: password });
        ...
        <div className="min-h-screen flex items-center justify-center bg-apple-gray-800 p-4">
            <Card title="Directory Admin Login" className="w-full max-w-sm">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Input label="Email or Username" id="username" value={username} onChange={(e) => setUsername(e.target.value)} required />
